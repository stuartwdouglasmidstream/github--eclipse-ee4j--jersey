...
set -- "$@" install -DallowIncompleteProjects -Danimal.sniffer.skip -Dcheckstyle.skip -Dcobertura.skip -Denforcer.skip -Dformatter.skip -Dgpg.skip -Dimpsort.skip -Djapicmp.skip -Dmaven.javadoc.failOnError=false -Dmaven.site.deploy.skip -Dpgpverify.skip -Drat.skip=true -Drevapi.skip -Dsort.skip -Dspotbugs.skip -Dspotless.check.skip=true -DskipTests -Dexamples.excluded org.apache.maven.plugins:maven-deploy-plugin:3.1.1:deploy 
...
if [ -n "" ]; then
  echo "Setting version to 3.1.1 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="3.1.1" | tee /root/project/logs/enforce-version.log
fi
